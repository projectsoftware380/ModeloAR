...
def procesar_matriz_indicadores(matriz_indicadores, estrategia_nan="reemplazar_por_0"):
    try:
        ...
        if matriz_indicadores.ndim == 3:
            ...
            logging.error(
                "La matriz de indicadores no es 1D ni 2D después de ajustar. "
                f"Dimensiones: {matriz_indicadores.ndim}"
            )
            return None
        ...
        return matriz_indicadores.flatten()
    except Exception as e:
...
CONFIG_PATH = Path(os.getenv("MODELOAR_CONFIG", str(BASE_DIR / "config4h.json")))
CURRENCY_PAIR = os.getenv("MODELOAR_CURRENCY_PAIR", "EURUSD")
MODEL_DIR = Path(
    os.getenv("MODELOAR_MODEL_DIR", str(BASE_DIR / CURRENCY_PAIR))
).expanduser()

# Configuración de logging
log_filename = BASE_DIR / "generate_trading_signals.log"
logging.basicConfig(
...
# Cargar configuración local (archivo excluido de Git)
if not CONFIG_PATH.exists():
    raise FileNotFoundError(
        f"No se encontró la configuración local en '{CONFIG_PATH}'. "
        "Copia config4h.example.json como config4h.json o define MODELOAR_CONFIG."
    )
...
def cargar_estado_modelo(modelo, ruta_modelo, dispositivo):
    ...
    if not ruta_modelo.exists():
        raise FileNotFoundError(f"No se encontró el artefacto de modelo: {ruta_modelo}")
...
# Cargar modelos entrenados desde una ruta portable/configurable
cargar_estado_modelo(actor, MODEL_DIR / "actor_model_final.pth", DEVICE)
cargar_estado_modelo(critic1, MODEL_DIR / "critic1_model_final.pth", DEVICE)
cargar_estado_modelo(critic2, MODEL_DIR / "critic2_model_final.pth", DEVICE)
...
# Cargar scaler y encoder desde el directorio del proyecto
with (BASE_DIR / "scaler.pkl").open("rb") as f:
    scaler = pickle.load(f)

with (BASE_DIR / "encoder.pkl").open("rb") as f:
...
def generar_señales(datos):
    try:
        ...
        datos[columnas_numericas] = (
            datos[columnas_numericas].apply(pd.to_numeric, errors="coerce").fillna(0)
        )
        ...
        datos["matriz_indicadores_deserialized"] = datos["matriz_indicadores"].apply(
            deserializar_matriz
        )
        if datos["matriz_indicadores_deserialized"].isnull().any():
            ...
            logging.info(
                "Filas con 'matriz_indicadores' inválidas eliminadas. "
                f"Total restante: {len(datos)}."
            )
            if len(datos) == 0:
                return pd.DataFrame()

        vectors = []
        for matriz_indicadores in datos["matriz_indicadores_deserialized"].values:
            vector = procesar_matriz_indicadores(
                matriz_indicadores, estrategia_nan="reemplazar_por_0"
            )
            if vector is None:
        ...
        if not all(col in datos.columns for col in required_signal_columns):
            logging.error(
                "Faltan columnas requeridas para construir precios SL/TP: "
                f"{required_signal_columns}"
            )
            return pd.DataFrame()

        estados_tensor = torch.tensor(estados, dtype=torch.float32).to(DEVICE)
        with torch.no_grad():
            medias, _ = actor(estados_tensor)
            acciones = medias
        ...
        señales = pd.DataFrame(
            {
                ...
                "sl_price": datos["close"]
                - (datos["atr_sl_size"] * datos["atr_norm"]),
                "tp_price": datos["close"]
                + (
                    datos["sl_atr_multiple"]
                    * (
                        datos["close"]
                        - (
                            datos["close"]
                ...
                "result_pips": np.nan,
                # La confianza no está calibrada en este experimento; se deja vacía
                # en lugar de generar un valor aleatorio que pudiera ser interpretado
                # como una métrica real del modelo.
                "confidence": np.nan,
                "result": np.nan,
                "status": "OPEN",
    ...
    try:
        ...
        if not señales.empty:
            with conn.cursor() as cursor:
                ...
                conn.commit()
                logging.info(
                    f"{len(señales)} señales insertadas en la tabla '{TABLA_SEÑALES}'."
                )
        else:
